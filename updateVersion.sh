...
if [[ "$OSTYPE" == "darwin"* ]]; then
  sed -E -i '' -e "${sed_script}" modules/servers/ispapissl/ispapissl.php
else
  sed -E -i -e "${sed_script}" modules/servers/ispapissl/ispapissl.php
fi
...
if [[ "$OSTYPE" == "darwin"* ]]; then
  sed -E -i '' -e "${sed_script}" modules/addons/ispapissl_addon/ispapissl_addon.php
else
  sed -E -i -e "${sed_script}" modules/addons/ispapissl_addon/ispapissl_addon.php
fi
...
if [[ "$OSTYPE" == "darwin"* ]]; then
  sed -E -i '' -e "${sed_script}" modules/servers/ispapissl/whmcs.json
else
  sed -E -i -e "${sed_script}" modules/servers/ispapissl/whmcs.json
fi
...
if [[ "$OSTYPE" == "darwin"* ]]; then
  sed -E -i '' -e "${sed_script}" modules/addons/ispapissl_addon/whmcs.json
else
  sed -E -i -e "${sed_script}" modules/addons/ispapissl_addon/whmcs.json
fi
